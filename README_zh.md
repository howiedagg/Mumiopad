...
#### 第一步：準備電腦端（PC Server）
1.  下載 `pc-server-python` 資料夾。
2.  點擊執行 `start.bat`。它會在背景執行，並在 Windows 右下角系統列出現圖示。
3.  *註：首次執行需要確保電腦已安裝 Python 3 以及相關套件。*

#### 第二步：準備手機端（Android App）
1.  安裝並開啟手機 App。
2.  確保手機與電腦連線至**同一個 Wi-Fi 網路**。
3.  App 會自動搜尋網路內的電腦。點擊您的電腦名稱，此時電腦螢幕會跳出配對詢問視窗，點擊「是」即可完成連線。
4.  您可以在電腦右下角系統匣圖示的「已信任裝置」選單中，單獨刪除或檢視已配對的手機，亦可在該選單一鍵清除所有紀錄。
